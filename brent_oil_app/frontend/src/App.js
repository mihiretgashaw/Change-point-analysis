...
function App() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5000/api/data?start_date=${dateRange.start}&end_date=${dateRange.end}`
      )
      .then((res) => setData(res.data));
    // Fetch LSTM forecast
    axios
      .get("http://localhost:5000/api/lstm_forecast")
      .then((res) => setForecast(res.data));
    // Fetch volatility
    axios
      .get("http://localhost:5000/api/volatility")
    ...
    <div className="App">
      ...
      <LineChart
        ...
        data={data.map((d, i) => ({
          ...
          price: d.Price,
          forecast:
            forecast.forecast && i >= data.length - forecast.forecast.length
              ? forecast.forecast[i - (data.length - forecast.forecast.length)]
              : null,
          gulf_war: d.Gulf_War,
      ...
      <LineChart
        width={800}
        height={400}
        data={volatility.dates.map((d, i) => ({
          date: d,
          volatility: volatility.volatility[i],
        }))}
      >
        <CartesianGrid strokeDasharray="3 3" />
        <XAxis dataKey="date" />
