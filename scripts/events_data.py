...
def plot_price_events(merged_data, figsize=(14, 7), price_color='blue', event_color='red',
    ...
    ax.plot(merged_data['Date'], merged_data['Price'], label='Brent Price', color=price_color)
    
    event_days = merged_data[merged_data['Event_Type'] != 'None']
    ax.scatter(event_days['Date'], event_days['Price'], color=event_color, label='Events', zorder=5)
...
if __name__ == "__main__":
    # Paths
    brent_filepath = "C:/Users/pc/Desktop/10 Academy/Week 10/Brentoil/data/brent_prices.csv"
    output_dir = "C:/Users/pc/Desktop/10 Academy/Week 10/Brentoil/data"
    
    # Load price data
    brent_df = load_brent_data(brent_filepath)
    
    # Create event data CSV and load DataFrame
    events_df = create_events_data(output_dir)
    ...
    # Merge event info to price data by nearest prior date (to handle weekends/holidays)
    merged_events = pd.merge_asof(events_df, brent_df, on='Date', direction='backward')
    
    # Merge full price data with merged_events info, left join to keep all prices
    merged_df = pd.merge(brent_df, merged_events[['Date', 'Event_Type', 'Event_Description']], on='Date', how='left')
    
    # Fill missing events with None or empty
    merged_df['Event_Type'].fillna('None', inplace=True)
    merged_df['Event_Description'].fillna('', inplace=True)
    
    print(f"Total price rows: {len(merged_df)}")
    print(f"Rows with events: {(merged_df['Event_Type'] != 'None').sum()}")
    
    # Plot price with event annotations
    fig = plot_price_events(merged_df)
    fig.savefig(os.path.join(output_dir, "brent_price_events.png"), dpi=300)
